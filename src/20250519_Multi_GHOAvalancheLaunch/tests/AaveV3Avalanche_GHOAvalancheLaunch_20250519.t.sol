...
contract AaveV3Avalanche_GHOAvalancheLaunch_20250519_Base is ProtocolV3TestBase {
    ...
    IEVM2EVMOnRamp internal constant ARB_ON_RAMP = IEVM2EVMOnRamp(GHOAvalancheLaunch.AVAX_ARB_ON_RAMP);
    IEVM2EVMOnRamp internal constant ETH_ON_RAMP = IEVM2EVMOnRamp(GHOAvalancheLaunch.AVAX_ETH_ON_RAMP);
    IEVM2EVMOnRamp internal constant BASE_ON_RAMP = IEVM2EVMOnRamp(GHOAvalancheLaunch.AVAX_BASE_ON_RAMP);
    IEVM2EVMOffRamp_1_5 internal constant ARB_OFF_RAMP = IEVM2EVMOffRamp_1_5(GHOAvalancheLaunch.AVAX_ARB_OFF_RAMP);
    IEVM2EVMOffRamp_1_5 internal constant ETH_OFF_RAMP = IEVM2EVMOffRamp_1_5(GHOAvalancheLaunch.AVAX_ETH_OFF_RAMP);
    IEVM2EVMOffRamp_1_5 internal constant BASE_OFF_RAMP = IEVM2EVMOffRamp_1_5(GHOAvalancheLaunch.AVAX_BASE_OFF_RAMP);
    ...
    IGhoAaveSteward internal constant NEW_GHO_AAVE_STEWARD = IGhoAaveSteward(GHOAvalancheLaunch.GHO_AAVE_CORE_STEWARD);
    IGhoBucketSteward internal constant NEW_GHO_BUCKET_STEWARD =
        IGhoBucketSteward(GHOAvalancheLaunch.GHO_BUCKET_STEWARD);
    IGhoCcipSteward internal constant NEW_GHO_CCIP_STEWARD = IGhoCcipSteward(GHOAvalancheLaunch.GHO_CCIP_STEWARD);
        ...
        returns (IClient.EVM2AnyMessage memory, IInternal.EVM2EVMMessage memory)
    {
        ...
        message.tokenAmounts[0] = IClient.EVMTokenAmount({token: address(GHO_TOKEN), amount: params.amount});
    ...
    AaveV3Avalanche_GHOAvalancheLaunch_20250519_Base
{
    ...
    function test_defaultProposalExecution() public {
        defaultTest("AaveV3Avalanche_GHOAvalancheLaunch_20250519", AaveV3Avalanche.POOL, address(proposal));
    }
    ...
    function test_stewardRoles() public {
        ...
        assertFalse(
            AaveV3Avalanche.ACL_MANAGER.hasRole(
                AaveV3Avalanche.ACL_MANAGER.RISK_ADMIN_ROLE(), address(NEW_GHO_AAVE_STEWARD)
            )
        );
        ...
        assertTrue(GHO_TOKEN.hasRole(GHO_TOKEN.FACILITATOR_MANAGER_ROLE(), GovernanceV3Avalanche.EXECUTOR_LVL_1));
        assertTrue(GHO_TOKEN.hasRole(GHO_TOKEN.BUCKET_MANAGER_ROLE(), GovernanceV3Avalanche.EXECUTOR_LVL_1));
        ...
        assertEq(facilitator.bucketCapacity, proposal.getCCIPBucketCapacity());
    ...
    function test_stewardsConfig() public view {
        ...
        assertEq(NEW_GHO_AAVE_STEWARD.POOL_ADDRESSES_PROVIDER(), address(AaveV3Avalanche.POOL_ADDRESSES_PROVIDER));
        assertEq(NEW_GHO_AAVE_STEWARD.POOL_DATA_PROVIDER(), address(AaveV3Avalanche.AAVE_PROTOCOL_DATA_PROVIDER));
        ...
        assertEq(IOwnable(address(NEW_GHO_BUCKET_STEWARD)).owner(), GovernanceV3Avalanche.EXECUTOR_LVL_1);
        ...
        assertEq(abi.encode(NEW_GHO_CCIP_STEWARD.getCcipTimelocks()), abi.encode(IGhoCcipSteward.CcipDebounce(0, 0)));
    }
    ...
    function test_newTokenPoolInitialization() public {
        ...
        vm.expectRevert("Initializable: contract is already initialized");
        IUpgradeableBurnMintTokenPool_1_5_1(IMPL).initialize(makeAddr("owner"), new address[](0), makeAddr("router"));
    ...
    function test_tokenPoolConfig() public {
        ...
        assertEq(NEW_TOKEN_POOL.getRemoteToken(ARB_CHAIN_SELECTOR), abi.encode(AaveV3ArbitrumAssets.GHO_UNDERLYING));
        assertEq(NEW_TOKEN_POOL.getRemoteToken(BASE_CHAIN_SELECTOR), abi.encode(AaveV3BaseAssets.GHO_UNDERLYING));
        ...
        assertEq(NEW_TOKEN_POOL.getRemotePools(BASE_CHAIN_SELECTOR)[0], abi.encode(NEW_REMOTE_POOL_BASE));

        assertEq(NEW_TOKEN_POOL.getCurrentInboundRateLimiterState(ETH_CHAIN_SELECTOR), _getRateLimiterConfig());
        assertEq(NEW_TOKEN_POOL.getCurrentOutboundRateLimiterState(ETH_CHAIN_SELECTOR), _getRateLimiterConfig());
        assertEq(NEW_TOKEN_POOL.getCurrentInboundRateLimiterState(ARB_CHAIN_SELECTOR), _getRateLimiterConfig());
        assertEq(NEW_TOKEN_POOL.getCurrentOutboundRateLimiterState(ARB_CHAIN_SELECTOR), _getRateLimiterConfig());
        assertEq(NEW_TOKEN_POOL.getCurrentInboundRateLimiterState(BASE_CHAIN_SELECTOR), _getRateLimiterConfig());
        assertEq(NEW_TOKEN_POOL.getCurrentOutboundRateLimiterState(BASE_CHAIN_SELECTOR), _getRateLimiterConfig());
    }
}

contract AaveV3Avalance_GHOAvalanceLaunch_20250519_PostExecution is AaveV3Avalanche_GHOAvalancheLaunch_20250519_Base {
    ...
    function test_sendMessageToArbSucceeds(uint256 amount) public {
        ...
        (IClient.EVM2AnyMessage memory message, IInternal.EVM2EVMMessage memory eventArg) =
            _getTokenMessage(CCIPSendParams({amount: amount, sender: alice, destChainSelector: ARB_CHAIN_SELECTOR}));
    ...
    function test_sendMessageToEthSucceeds(uint256 amount) public {
        ...
        (IClient.EVM2AnyMessage memory message, IInternal.EVM2EVMMessage memory eventArg) =
            _getTokenMessage(CCIPSendParams({amount: amount, sender: alice, destChainSelector: ETH_CHAIN_SELECTOR}));
    ...
    function test_sendMessageToBaseSucceeds(uint256 amount) public {
        ...
        (IClient.EVM2AnyMessage memory message, IInternal.EVM2EVMMessage memory eventArg) =
            _getTokenMessage(CCIPSendParams({amount: amount, sender: alice, destChainSelector: BASE_CHAIN_SELECTOR}));
    ...
    function test_cannotOffRampOtherChainMessages() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(InvalidSourcePoolAddress.selector, abi.encode(address(NEW_REMOTE_POOL_ETH)))
        );
        vm.prank(address(ARB_OFF_RAMP));
        NEW_TOKEN_POOL.releaseOrMint(
            IPool_CCIP.ReleaseOrMintInV1({
        ...
        vm.expectRevert(
            abi.encodeWithSelector(InvalidSourcePoolAddress.selector, abi.encode(address(NEW_REMOTE_POOL_ARB)))
        );
        vm.prank(address(ETH_OFF_RAMP));
        NEW_TOKEN_POOL.releaseOrMint(
            IPool_CCIP.ReleaseOrMintInV1({
