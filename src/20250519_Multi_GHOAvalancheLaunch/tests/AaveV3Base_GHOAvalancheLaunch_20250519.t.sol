 ...
 * command: FOUNDRY_PROFILE=test forge test --match-path=src/20250419_Multi_GHOAvalancheLaunch/Base_Avalanche_AaveV3GHOLane_20250519.t.sol -vv
 */
contract Base_Avalanche_AaveV3GHOLane_20250519_Avalanche is ProtocolV3TestBase {
    ...
    IEVM2EVMOnRamp internal constant ETH_ON_RAMP = IEVM2EVMOnRamp(GHOAvalancheLaunch.BASE_ETH_ON_RAMP);
    IEVM2EVMOnRamp internal constant AVAX_ON_RAMP = IEVM2EVMOnRamp(GHOAvalancheLaunch.BASE_AVAX_ON_RAMP);
    IEVM2EVMOffRamp_1_5 internal constant ETH_OFF_RAMP = IEVM2EVMOffRamp_1_5(GHOAvalancheLaunch.BASE_ETH_OFF_RAMP);
    IEVM2EVMOffRamp_1_5 internal constant AVAX_OFF_RAMP = IEVM2EVMOffRamp_1_5(GHOAvalancheLaunch.BASE_AVAX_OFF_RAMP);
    ...
    IGhoCcipSteward internal constant NEW_GHO_CCIP_STEWARD = IGhoCcipSteward(GhoBase.GHO_CCIP_STEWARD);
    ...
    Base_Avalanche_AaveV3GHOLane_20250519 internal proposal;
    ...
    function setUp() public virtual {
        ...
        proposal = new Base_Avalanche_AaveV3GHOLane_20250519();
        // _validateConstants();
    }
...
contract Base_Avalanche_AaveV3GHOLane_20250519_PostExecution is Base_Avalanche_AaveV3GHOLane_20250519_Avalanche {
    ...
    function test_basePoolConfig() public view {
        ...
        assertEq(
            NEW_TOKEN_POOL.getRemoteToken(ETH_CHAIN_SELECTOR), abi.encode(address(AaveV3EthereumAssets.GHO_UNDERLYING))
        );
        assertEq(
            NEW_TOKEN_POOL.getRemoteToken(ARB_CHAIN_SELECTOR), abi.encode(address(AaveV3ArbitrumAssets.GHO_UNDERLYING))
        );
        assertEq(NEW_TOKEN_POOL.getRemoteToken(AVAX_CHAIN_SELECTOR), abi.encode(address(NEW_REMOTE_TOKEN_AVAX)));

        assertEq(NEW_TOKEN_POOL.getRemotePools(ARB_CHAIN_SELECTOR).length, 1);
        assertEq(NEW_TOKEN_POOL.getRemotePools(ARB_CHAIN_SELECTOR)[0], abi.encode(address(NEW_REMOTE_POOL_ARB)));
        ...
        assertEq(NEW_TOKEN_POOL.getRemotePools(AVAX_CHAIN_SELECTOR)[0], abi.encode(address(NEW_REMOTE_POOL_AVAX)));

        // Omit checking rate limit configs against other chains because it's dynamic and not an area of concern for this AIP
        assertEq(NEW_TOKEN_POOL.getCurrentInboundRateLimiterState(AVAX_CHAIN_SELECTOR), _getRateLimiterConfig());
        assertEq(NEW_TOKEN_POOL.getCurrentOutboundRateLimiterState(AVAX_CHAIN_SELECTOR), _getRateLimiterConfig());
    }

    function test_sendMessageToAvaxSucceeds(uint256 amount) public {
        uint256 bridgeableAmount =
            _min(GHO.getFacilitator(address(NEW_TOKEN_POOL)).bucketLevel, CCIP_RATE_LIMIT_CAPACITY);
        ...
        (IClient.EVM2AnyMessage memory message, IInternal.EVM2EVMMessage memory eventArg) =
            _getTokenMessage(CCIPSendParams({amount: amount, sender: alice, destChainSelector: AVAX_CHAIN_SELECTOR}));
    ...
    function test_sendMessageToEthSucceeds(uint256 amount) public {
        IRateLimiter.TokenBucket memory ethRateLimits =
            NEW_TOKEN_POOL.getCurrentInboundRateLimiterState(ETH_CHAIN_SELECTOR);
        uint256 bridgeableAmount = _min(GHO.getFacilitator(address(NEW_TOKEN_POOL)).bucketLevel, ethRateLimits.capacity);
        amount = bound(amount, 1, bridgeableAmount);
        skip(_getOutboundRefillTime(amount)); // wait for the rate limiter to refill
    ...
    function test_offRampViaAvaxSucceeds(uint256 amount) public {
        (uint256 bucketCapacity, uint256 bucketLevel) = GHO.getFacilitatorBucket(address(NEW_TOKEN_POOL));
    ...
    function test_cannotUseAvaxOffRampForEthMessages() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(CallerIsNotARampOnRouter.selector, address(AVAX_OFF_RAMP)));
        vm.prank(address(AVAX_OFF_RAMP));
        NEW_TOKEN_POOL.releaseOrMint(
            IPool_CCIP.ReleaseOrMintInV1({
                originalSender: abi.encode(alice),
    ...
    function test_cannotOffRampOtherChainMessages() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(InvalidSourcePoolAddress.selector, abi.encode(address(NEW_REMOTE_POOL_AVAX)))
        );
        vm.prank(address(ETH_OFF_RAMP));
        NEW_TOKEN_POOL.releaseOrMint(
            IPool_CCIP.ReleaseOrMintInV1({
