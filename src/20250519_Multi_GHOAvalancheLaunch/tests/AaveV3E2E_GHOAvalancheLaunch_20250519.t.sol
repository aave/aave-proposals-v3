...
import {IUpgradeableLockReleaseTokenPool_1_5_1} from
    "src/interfaces/ccip/tokenPool/IUpgradeableLockReleaseTokenPool.sol";
...
contract Base_Avalanche_AaveV3GHOLane_20250519_Base is ProtocolV3TestBase {
    ...
    uint128 public constant CCIP_RATE_LIMIT_REFILL_RATE = GHOAvalancheLaunch.CCIP_RATE_LIMIT_REFILL_RATE;
    ...
    function setUp() public virtual {
        ...
        base.tokenPool = IUpgradeableBurnMintTokenPool_1_5_1(GHOAvalancheLaunch.BASE_GHO_CCIP_TOKEN_POOL);
        ...
        eth.tokenPool = IUpgradeableLockReleaseTokenPool_1_5_1(GHOAvalancheLaunch.ETH_GHO_CCIP_TOKEN_POOL);
    ...
    function _getTokenMessage(CCIPSendParams memory params)
        internal
        returns (IClient.EVM2AnyMessage memory, IInternal.EVM2EVMMessage memory)
    {
        IClient.EVM2AnyMessage memory message = CCIPUtils.generateMessage(params.sender, 1);
        message.tokenAmounts[0] = IClient.EVMTokenAmount({token: address(params.src.token), amount: params.amount});
    ...
    function _validateConfig(bool executed) internal {
        ...
        if (executed) {
            ...
            assertEq(arb.tokenPool.getRemoteToken(base.c.chainSelector), abi.encode(address(base.c.token)));
            assertEq(arb.tokenPool.getRemotePools(ava.c.chainSelector).length, 1);
            assertEq(arb.tokenPool.getRemotePools(ava.c.chainSelector)[0], abi.encode(address(ava.tokenPool)));
            ...
            assertEq(arb.tokenPool.getRemotePools(base.c.chainSelector)[0], abi.encode(address(base.tokenPool)));
            ...
            assertEq(base.tokenPool.getRemoteToken(eth.c.chainSelector), abi.encode(address(eth.c.token)));
            assertEq(base.tokenPool.getRemoteToken(ava.c.chainSelector), abi.encode(address(ava.c.token)));
            assertEq(base.tokenPool.getRemoteToken(arb.c.chainSelector), abi.encode(address(arb.c.token)));
            assertEq(base.tokenPool.getRemotePools(ava.c.chainSelector).length, 1);
            assertEq(base.tokenPool.getRemotePools(ava.c.chainSelector)[0], abi.encode(address(ava.tokenPool)));
            assertEq(base.tokenPool.getRemotePools(eth.c.chainSelector).length, 1);
            assertEq(base.tokenPool.getRemotePools(eth.c.chainSelector)[0], abi.encode(address(eth.tokenPool)));
            assertEq(base.tokenPool.getRemotePools(arb.c.chainSelector).length, 1);
            assertEq(base.tokenPool.getRemotePools(arb.c.chainSelector)[0], abi.encode(address(arb.tokenPool)));
            ...
            assertEq(ava.tokenPool.getRemoteToken(base.c.chainSelector), abi.encode(address(base.c.token)));
            assertEq(ava.tokenPool.getRemotePools(arb.c.chainSelector).length, 1);
            assertEq(ava.tokenPool.getRemotePools(arb.c.chainSelector)[0], abi.encode(address(arb.tokenPool)));
            assertEq(ava.tokenPool.getRemotePools(eth.c.chainSelector).length, 1);
            assertEq(ava.tokenPool.getRemotePools(eth.c.chainSelector)[0], abi.encode(address(eth.tokenPool)));
            assertEq(ava.tokenPool.getRemotePools(base.c.chainSelector).length, 1);
            assertEq(ava.tokenPool.getRemotePools(base.c.chainSelector)[0], abi.encode(address(base.tokenPool)));
            ...
            assertEq(eth.tokenPool.getRemoteToken(base.c.chainSelector), abi.encode(address(base.c.token)));
            ...
            assertEq(eth.tokenPool.getRemotePools(base.c.chainSelector)[0], abi.encode(address(base.tokenPool)));
            assertEq(eth.tokenPool.getRemotePools(ava.c.chainSelector).length, 1);
            assertEq(eth.tokenPool.getRemotePools(ava.c.chainSelector)[0], abi.encode(address(ava.tokenPool)));
        }
    }

    function _assertOnRamp(IEVM2EVMOnRamp onRamp, uint64 srcSelector, uint64 dstSelector, IRouter router)
        internal
        view
    {
    ...
    function _assertOffRamp(IEVM2EVMOffRamp_1_5 offRamp, uint64 srcSelector, uint64 dstSelector, IRouter router)
        internal
        view
    {
    ...
    function _assertSetRateLimit(Common memory src, address tokenPool) internal view {
        ...
        IUpgradeableLockReleaseTokenPool_1_5_1 _tokenPool = IUpgradeableLockReleaseTokenPool_1_5_1(tokenPool);
        assertEq(_tokenPool.getCurrentInboundRateLimiterState(dst1.chainSelector), _getRateLimiterConfig());
        assertEq(_tokenPool.getCurrentOutboundRateLimiterState(dst1.chainSelector), _getRateLimiterConfig());

        assertEq(_tokenPool.getCurrentInboundRateLimiterState(dst2.chainSelector), _getRateLimiterConfig());
        assertEq(_tokenPool.getCurrentOutboundRateLimiterState(dst2.chainSelector), _getRateLimiterConfig());
    }
    ...
    function _tokenBucketToConfig(IRateLimiter.TokenBucket memory bucket)
        internal
        pure
        returns (IRateLimiter.Config memory)
    {
        return IRateLimiter.Config({isEnabled: bucket.isEnabled, capacity: bucket.capacity, rate: bucket.rate});
    }
    ...
    function assertEq(IRateLimiter.TokenBucket memory bucket, IRateLimiter.Config memory config) internal pure {
    ...
    function _refreshGasAndTokenPrices(Common memory src, Common memory dst) internal {
        ...
        priceRegistry.updatePrices(
            IInternal.PriceUpdates({tokenPriceUpdates: tokenPriceUpdates, gasPriceUpdates: gasPriceUpdates})
        );
    }
}

contract Base_Avalanche_AaveV3GHOLane_20250519_PostExecution is Base_Avalanche_AaveV3GHOLane_20250519_Base {
    ...
    function test_E2eEthAvax(uint256 amount) public {
        {
            vm.selectFork(eth.c.forkId);
            uint256 bridgeableAmount =
                _min(eth.tokenPool.getBridgeLimit() - eth.tokenPool.getCurrentBridgedAmount(), CCIP_RATE_LIMIT_CAPACITY);
            ...
            (IClient.EVM2AnyMessage memory message, IInternal.EVM2EVMMessage memory eventArg) =
    ...
    function test_E2eArbAvax(uint256 amount) public {
        {
            vm.selectFork(arb.c.forkId);
            IRateLimiter.TokenBucket memory avaRateLimits =
                arb.tokenPool.getCurrentInboundRateLimiterState(ava.c.chainSelector);
            uint256 bridgeableAmount =
                _min(arb.c.token.getFacilitator(address(arb.tokenPool)).bucketLevel, avaRateLimits.capacity);
            amount = bound(amount, 1, bridgeableAmount);
            skip(_getOutboundRefillTime(amount));
            ...
            assertEq(arb.c.token.getFacilitator(address(arb.tokenPool)).bucketLevel, facilitatorLevel - amount);
    ...
    function test_E2eBaseAvax(uint256 amount) public {
        {
            vm.selectFork(base.c.forkId);
            IRateLimiter.TokenBucket memory avaRateLimits =
                base.tokenPool.getCurrentInboundRateLimiterState(ava.c.chainSelector);
            uint256 bridgeableAmount =
                _min(base.c.token.getFacilitator(address(base.tokenPool)).bucketLevel, avaRateLimits.capacity);
            amount = bound(amount, 1, bridgeableAmount);
            skip(_getOutboundRefillTime(amount));
            ...
            (IClient.EVM2AnyMessage memory message, IInternal.EVM2EVMMessage memory eventArg) =
                _getTokenMessage(CCIPSendParams({src: base.c, dst: ava.c, sender: alice, amount: amount}));
            ...
            assertEq(base.c.token.getFacilitator(address(base.tokenPool)).bucketLevel, facilitatorLevel - amount);
        ...
        // send amount back to avalanche
        {
            ...
            (IClient.EVM2AnyMessage memory message, IInternal.EVM2EVMMessage memory eventArg) =
                _getTokenMessage(CCIPSendParams({src: ava.c, dst: base.c, sender: alice, amount: amount}));
    ...
    function test_E2eEthArb(uint256 amount) public {
        {
            vm.selectFork(eth.c.forkId);
            IRateLimiter.TokenBucket memory rateLimits =
                eth.tokenPool.getCurrentInboundRateLimiterState(arb.c.chainSelector);
            uint256 bridgeableAmount =
                _min(eth.tokenPool.getBridgeLimit() - eth.tokenPool.getCurrentBridgedAmount(), rateLimits.capacity);
            amount = bound(amount, 1, bridgeableAmount);
            skip(_getOutboundRefillTime(amount));
            ...
            assertEq(arb.c.token.balanceOf(alice), aliceBalance + amount);
            assertEq(arb.c.token.getFacilitator(address(arb.tokenPool)).bucketLevel, bucketLevel + amount);
        }
