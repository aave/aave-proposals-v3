 ...
 * command: FOUNDRY_PROFILE=test forge test --match-path=src/20250519_Multi_GHOAvalancheLaunch/Ethereum_Avalanche_AaveV3GHOLane_20250519.t.sol -vv
 */
contract Ethereum_Avalanche_AaveV3GHOLane_20250519_Avalanche is ProtocolV3TestBase {
    ...
    IEVM2EVMOnRamp internal constant BASE_ON_RAMP = IEVM2EVMOnRamp(GHOAvalancheLaunch.ETH_BASE_ON_RAMP);
    IEVM2EVMOnRamp internal constant AVAX_ON_RAMP = IEVM2EVMOnRamp(GHOAvalancheLaunch.ETH_AVAX_ON_RAMP);
    IEVM2EVMOffRamp_1_5 internal constant BASE_OFF_RAMP = IEVM2EVMOffRamp_1_5(GHOAvalancheLaunch.ETH_BASE_OFF_RAMP);
    IEVM2EVMOffRamp_1_5 internal constant AVAX_OFF_RAMP = IEVM2EVMOffRamp_1_5(GHOAvalancheLaunch.ETH_AVAX_OFF_RAMP);
    ...
    IGhoCcipSteward internal constant NEW_GHO_CCIP_STEWARD = IGhoCcipSteward(GhoEthereum.GHO_CCIP_STEWARD);
    ...
    Ethereum_Avalanche_AaveV3GHOLane_20250519 internal proposal;
    ...
    function setUp() public {
        ...
        proposal = new Ethereum_Avalanche_AaveV3GHOLane_20250519();
        // _validateConstants();
    ...
    function test_basePoolConfig() public view {
        ...
        assertEq(
            NEW_TOKEN_POOL.getRemoteToken(BASE_CHAIN_SELECTOR), abi.encode(address(AaveV3BaseAssets.GHO_UNDERLYING))
        );
        ...
        assertEq(NEW_TOKEN_POOL.getRemotePools(BASE_CHAIN_SELECTOR)[0], abi.encode(address(NEW_REMOTE_POOL_BASE)));
    ...
    function test_sendMessageToAvaxSucceeds(uint256 amount) public {
        uint256 bridgeableAmount =
            _min(NEW_TOKEN_POOL.getBridgeLimit() - NEW_TOKEN_POOL.getCurrentBridgedAmount(), CCIP_RATE_LIMIT_CAPACITY);
    ...
    function test_sendMessageToBaseSucceeds(uint256 amount) public {
        IRateLimiter.TokenBucket memory baseRateLimits =
            NEW_TOKEN_POOL.getCurrentInboundRateLimiterState(BASE_CHAIN_SELECTOR);
        uint256 bridgeableAmount =
            _min(NEW_TOKEN_POOL.getBridgeLimit() - NEW_TOKEN_POOL.getCurrentBridgedAmount(), baseRateLimits.capacity);
    ...
    function test_offRampViaAvaxSucceeds(uint256 amount) public {
        uint256 bridgeableAmount = _min(NEW_TOKEN_POOL.getCurrentBridgedAmount(), CCIP_RATE_LIMIT_CAPACITY);
        amount = bound(amount, 1, bridgeableAmount);
    ...
    function test_cannotOffRampOtherChainMessages() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(InvalidSourcePoolAddress.selector, abi.encode(address(NEW_REMOTE_POOL_BASE)))
        );
        vm.prank(address(AVAX_OFF_RAMP));
        NEW_TOKEN_POOL.releaseOrMint(
            IPool_CCIP.ReleaseOrMintInV1({
