...
LlamaRisk recommends an eighth round of Add Cap and Draw Cap increases for Aave V4. Deposits have continued to grow, reaching approximately $183M in total, with inflows remaining concentrated in BTC and ETH collateral on the Core Hub. Notably, WBTC on Core Main has already refilled the capacity added in the previous round and has returned to full Add Cap utilization, reflecting continued depositor demand. Round 8 expands capacity where utilization indicates additional headroom is warranted.
The proposed adjustments add approximately $36M in additional Add Cap capacity, all on the Core Hub. Total Add Cap ceiling moves from approximately $392M to $427M. Draw Caps on affected assets are scaled proportionally to preserve existing Add/Draw ratios.
...
Round 8 targets approximately $36M in additional Add Cap capacity, all on the Core Hub, led by the WBTC increase. USDG and frxUSD on Core Main are held at their current 30M caps.
