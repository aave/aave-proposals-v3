...
discussions: "https://governance.aave.com/t/arfc-aave-v4-activation-on-ethereum-mainnet/24293/49"
---

## Summary

LlamaRisk recommends a sixteenth round of adjustments for Aave V4. Following the execution of Round 15, total deposits across the five hubs on two chains stand at approximately $577M, with the Global Dollar Hub reaching approximately $94M and the Plus Hub approximately $21M. This round includes a single collateral increase on the Plus Hub. The primary changes are on the borrow side, with increases to several Core borrow lines to the Prime and Plus Hubs that have reached full utilization. Several of these lines are driven by the ongoing USDC borrow rewards campaign on the Core and Prime Hubs. USDG is also enabled as a new borrowable reserve on the Prime Hub Bluechip Spoke through a credit line extended from the Core Hub. IRM changes for USDC on the Core/Prime Hub and USDe on Plus Hub have also been proposed.
...
On the Prime Hub, USDG is introduced as a new borrowable reserve through a credit line originating from the Core Hub. The Bluechip Spoke has attracted a growing base of blue-chip collateral, with WBTC ($19.6M), wstETH ($16.3M), cbBTC ($10.8M), and WETH ($4.2M) in deposits. These assets currently support borrowing of USDC, USDT, GHO, and frxUSD, leaving USDG as the only broadly available stablecoin on Aave V4 that is not yet borrowable through the spoke.

The new reserve operates consistently with the existing Bluechip stablecoin credit lines for USDC, USDT, and frxUSD, drawing from Core inventory that is sufficiently deep relative to the proposed cap. A conservative 5M initial credit line follows the approach used for those lines and can be increased as utilization develops.

On the Avalanche Core Hub, caps remain unchanged given assets’ thin onchain liquidity.
...
Round 16 targets approximately $7M of additional Add Cap capacity on Ethereum Plus, alongside draw-side relief for the most utilized borrow lines and a new USDG credit line to the Prime Hub Bluechip Spoke. IRM changes for USDC and USDe are also proposed.
